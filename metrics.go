...
func (m *metricExchange) requestsLocked() *Number {
...
func (m *metricExchange) Requests() *Number {
	m.Mutex.RLock()
	defer m.Mutex.RUnlock()
...
func newMetricExchange(name string) *metricExchange {
	...
	regist := new(metricCollectorRegistry)
	m.metricCollectors = regist.InitializeMetricCollectors(name)
	m.Reset()
