...
func (circuit *CircuitBreaker) setOpen() {
	...
	log.Printf("bxd-go: opening circuit %v", circuit.Name)
...
func (circuit *CircuitBreaker) setClose() {
	...
	log.Printf("bxd-go: closing circuit %v", circuit.Name)
...
func (circuit *CircuitBreaker) allowSingleTest() bool {
	...
	if circuit.open && now > openedOrLastTestedTime+getSettings(circuit.Name).SleepWindow.Nanoseconds() {
		...
		if swapped {
			log.Printf("bxd-go: allowing single test to possibly close circuit %v", circuit.Name)
		}
		return swapped
	}
